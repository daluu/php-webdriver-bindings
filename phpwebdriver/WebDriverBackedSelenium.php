 ...
 *  
 *  NOTE: This version is for use outside of PHPUnit, in regular
 *  PHP code or via a PHP shell session. This class is not for 
 *  execution via PHPUnit directly (w/o wrappers, etc.). For that,
 *  use WebDriverBackedSeleniumTestCase.php.
 *  
 *  Base class for functional tests using WebDriver with PHP.
 *  It provides the same interface as PHPUnit's SeleniumTestCase,
 *  intending to allow users to more easily port existing PHP 
 *  Selenium (RC) tests to Web Driver with minimal or no code 
 *  changes. And/or export Selenium IDE tests (for Selenium RC)
 *  to PHP for WebDriver w/o much rework.
 *  
...
class WebDriverBackedSelenium{
	...
	function __construct( $host="localhost", $port="4444", $browser="firefox" ) {
    ...
    */
	...
	private function getElements( $strategy, $name ) {    	
        ...
        if( !isset( $elements ) )
            throw new Exception("Elements have not appeared after " . $this->max_waiting_time . " seconds.");
        return $elements;
    }
